...
from os import path
import csv
from .models import RANDCROP1, RANDFLIP1, RANDCOLOR1, BRIGHTNESS1, CONTRAST1
    ...
    def __getitem__(self, idx):
        if self.aug_flag:
            with Image.open(path.join(self.short_path, self.img_labels[idx][0])) as im:
                t = []
                if RANDCROP1:
                    t.append(transforms.RandomCrop((64, 64)))
                if RANDFLIP1:
                    t.append(transforms.RandomHorizontalFlip())
                if RANDCOLOR1:
                    transforms.ColorJitter(brightness=BRIGHTNESS1, contrast=CONTRAST1)
